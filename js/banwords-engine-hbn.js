(function(global){
  ...
  function sanitizeAllowedCharacters(text, role){
    ...
    } else if (role === 'host' || role === '購物專家') {
      /* 購物專家（主播 Bar）：在一般白名單之外，額外放行分隔符「｜」與「|」，
         用於「購物專家｜姓名」格式。
         防呆：全形（U+FF5C）與半形（U+007C）都列入，
              避免使用者手打半形分隔符時又被清空。
         ★ 同時接受兩個角色別名：畫布編輯路徑（bn.html roleMap3）用 'host'，
           右側工具列輸入框（bn-state FIELD_MAP）用 '購物專家'——兩條路徑都要放行，
           否則會出現「畫布編輯正常、但工具列欄位編輯分隔符消失」的分歧 BUG。
         注意：此分隔符「只」對此角色開放，主標/副標/品牌名仍走下方 else，不受影響。 */
      out = out.replace(/[^\p{Script=Han}\p{L}\p{N}\s,$%｜|]/gu, '');
    } else {
      out = out.replace(/[^\p{Script=Han}\p{L}\p{N}\s,$%]/gu, '');
    }
